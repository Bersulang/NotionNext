...
export default async function getPageProperties(
  ...
  normalizeBackfilledPostDates(properties, value)
...
/**
 * 2026-07-04 批量补录历史文章时，Notion 的真实编辑日期会暴露为补录日。
 * 这批文章只展示用户手动设置的发布时间；后续正常发布/编辑仍保留原始逻辑。
 */
function normalizeBackfilledPostDates(properties, value) {
  const backfillStart = new Date('2026-07-03T16:00:00.000Z').getTime()
  const backfillEnd = new Date('2026-07-04T16:00:00.000Z').getTime()
  const lastEditedTime = new Date(value?.last_edited_time).getTime()
  const hasExplicitPublishDate = Boolean(properties?.date?.start_date)
  const isBackfilledToday =
    lastEditedTime >= backfillStart && lastEditedTime < backfillEnd

  if (
    properties.type === 'Post' &&
    hasExplicitPublishDate &&
    isBackfilledToday
  ) {
    properties.lastEditedDate = new Date(properties.publishDate)
    properties.lastEditedDay = properties.publishDay
  }
}
